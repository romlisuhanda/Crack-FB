...
module Facebook
  ...
  class Crack < Login
    ...
    def initialize(listid:,max_workers: 30)
      ...
      if auto == 'm'
        puts("\n[!] Use ',' As a separator, for example: Dog,Honey")
        loop do
    ...
    def start
      puts ("\n[!] Results OK Saved In : #{@ok_save}")
      puts ("[!] Results CP Saved In : #{@cp_save}\n\n")
      ok = 0
      cp = 0
      @email.each do |usr|
        @pool.schedule do
...
def main
  ...
  puts ("[✓] IP ADDRESS  : #{$ip}\n\n")
  puts ("[1] Crack From Friends")
  puts ("[2] Crack From Followers")
  puts ("[3] Crack From Following")
  puts ("[4] Crack From Like Post")
  puts ("[5] Crack From Friends,Followers,Following")
  puts ("[6] View Crack Results")
  puts ("[7] Settings User Agent")
  puts ("[8] Logout (delete token)")
  puts ("[0] Exit\n\n")
  ...
  case pilih
    ...
    when 6
      ...
      puts ("\n[1] View Results OK (#{ok.length})")
      puts ("[2] View Results CP (#{cp.length})")
      puts ("[0] Return\n\n")
      print ("[?] Select : ")
      ...
      if naon == 1 and !ok.empty?
        ...
        puts ("\n[!] You Have No Results CP")
      else
        main()
      end
    when 7
      puts ("\n[1] Check User Agent")
      puts ("[2] Change User Agent")
      puts ("[0] Return\n\n")
      print ("[?] Select : ")
